...
module tinyqv_core #(parameter NUM_REGS=16, parameter REG_ADDR_BITS=4) (
    ...
    reg mstatus_mie;   // Interrupt enable (kept for MRET round-trip)
...
/*verilator lint_on UNUSEDSIGNAL*/
    ...
    tinyqv_alu i_alu(alu_op, alu_a_in, alu_b_in, cy_in, cmp_in, alu_out, cy_out, cmp_out);
    ...
    assign branch = last_count && ((is_jal || is_jalr || is_trap || is_mret) || (is_branch && take_branch));
    ...
    always @(*) begin
        instr_complete = 0;
        if (last_count) begin
            if (is_auipc || is_lui || is_store || is_jal || is_jalr || is_system || is_stall || is_trap || is_branch)
                instr_complete = 1;
    ...
    always @(*) begin
        tmp_data_shift = 1;
        if (is_trap)
            tmp_data_in = (counter == 0) ? {1'b0, is_trap && mstatus_mte, 2'b00} : 4'b0000;
        else if (is_shift)
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            mstatus_mte <= 1;
        end else if (counter == 0 && is_trap) begin
            mstatus_mte <= 0;
    ...
    always @(*) begin
        case (imm_lo)
            ...
            // misa: RV32E
            12'h301: csr_read = (counter == 0 || counter == 7) ? 4'b0100 :  // 32
                                (counter == 1) ?                 4'b0001 :  // E
                                                                 4'b0000;
